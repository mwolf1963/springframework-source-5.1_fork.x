...
public class InternalResourceView extends AbstractUrlBasedView {
	...
	protected void renderMergedOutputModel(
		...
		// 将请求模型model中的key value值封装到request域对象中，所以跳转到页面上之后，可以取出请求域中的属性值
		exposeModelAsRequestAttributes(model, request);

		// 这是一个扩展点，可以暴露JSTL中的一些特有属性到视图页面上，例如时区信息
		...
		// 获取请求转发器RequestDispatcher。请求转发器就是Servlet中使用request.getRequestDispatcher(path)获取到的对象
